...
def run(args: argparse.Namespace) -> None:
    ...
    print(f"[s_end_v2] LookaheadAdvisor: 12-ply width, {args.sim_ply_depth}-ply sim, 5 signals (h+learner_sent+opp_sent+vn+gap), early-exit DB: {_la_src}")

    # ── SpecialistDB ─────────────────────────────────────────────────────────
    specialist_db = SpecialistDB(_ROOT / "data" / "specialist_db.sqlite")
    print(f"[s_end_v2] SpecialistDB: {specialist_db.stats()}")
    ...
    print(f"[s_end_v2] feat_dim={MOVE_FEAT_DIM_WITH_LOOKAHEAD}, starting game={start_game}, diff={difficulty}")
                    ...
                    if prev_best.exists():
                        ...
                        try:
                            ...
                            try:
                                ...
                                print(f"[s_end_v2] Advance-load: checkpoint shape incompatible (old feat_dim) — keeping current weights")
                        model.to(device)
