"""scripts/train_s_end_v2.py — Endgame specialist v2: 15-ply lookahead + gap net.

Trains on positions where < 12 total pieces are on the board or fly phase is
active.  Reward: sentinel delta + heuristic delta + mill bonus; Malom = 0.
Gap net included in lookahead (15-ply × 4 signals).  Malom DB used for
lookahead rollout termination only (no Malom reward signal).

Resume chain: explicit --resume → s_end_v2/best.pt → scratch

Usage
-----
.venv/bin/python scripts/train_s_end_v2.py --max-games 20 --max-branches-per-game 0
...
STAGE_TAG = "s_end_v2"
OUT_DIR   = "learned_ai/checkpoints/scaffolded/s_end_v2"

# ── Reward weights ────────────────────────────────────────────────────────────

ALPHA      = 0.20   # sentinel quality delta
BETA       = 0.15   # heuristic delta
MILL_BONUS = 0.15   # mill formation (reduced vs midgame)
...
def _run_s1b_refresher(
    ...
    p = Path(data_path)
    if not p.exists():
        print(f"[s_end_v2] s1b refresher: data not found ({data_path}) — skipping")
        return
    ...
    model.train()
    print(f"[s_end_v2] s1b refresher: loser={len(loser_idxs)} winner={len(winner_idxs)} positions  lr={lr:.2e}")

    def _pad_feat(fm: np.ndarray) -> np.ndarray:
        k, d = fm.shape
        if d >= MOVE_FEAT_DIM_WITH_TOPK:
            return fm[:, :MOVE_FEAT_DIM_WITH_TOPK]
        pad = np.zeros((k, MOVE_FEAT_DIM_WITH_TOPK - d), dtype=np.float32)
    ...
    def _run_phase(phase_idxs: list[int], phase_label: str, use_heuristic_target: bool) -> None:
        ...
        for epoch in range(1, epochs + 1):
            ...
            print(f"[s_end_v2]   refresher [{phase_label}] epoch {epoch}/{epochs}  loss={ep_loss / max(ep_w_sum, 1e-9):.4f}")
...
def _choose_resume_path(args: argparse.Namespace) -> tuple[Optional[Path], str]:
    ...
    s_end_v2_best = _ROOT / "learned_ai" / "checkpoints" / "scaffolded" / "s_end_v2" / "best.pt"
    if args.auto_resume_best and s_end_v2_best.exists():
        return s_end_v2_best, "s_end_v2_best"
    return None, "scratch"
...
def _compute_per_move_reward(
    ...
    in_endgame = total_pieces < 12 or board_phase not in ("place", "move")
...
def _retroactive_rescore(trajectory: list[ScaffoldedStep], step_diags: list[StepDiag], outcome: float) -> None:
    ...
    for t_idx, step in enumerate(trajectory):
        ...
        # Malom-agreement bonus: +ve when chosen move improves Malom WDL (endgame only).
        mq = step_diags[t_idx].malom_chosen_dtm
        if mq is not None:
            delta += MALOM_AGREE_BONUS * float(mq)
        step.reward     += delta
...
def _rollout(
    ...
    while ply < max_ply:
        ...
        if player == learner_color:
            # v3: top-K candidates from alpha-beta + lookahead + human prior.
            ...
            if enc_after is not None and enc_after.legal_moves:
                # enc_after is 122-float base+lookahead; zero-pad to 126 to match spec input.
            ...
            # Malom quality of chosen move: +ve if improved WDL, -ve if degraded, None if unknown
            _malom_q: Optional[float] = None
            if endgame_db is not None:
                try:
                    _malom_q = endgame_db.query_move_quality(board, move)
                except Exception:
                    _malom_q = None
...
def run(args: argparse.Namespace) -> None:
    ...
    print(f"[s_end_v2] Device: {device}")
    ...
    if Path(sent_path).exists():
        ...
        if sentinel and sentinel.is_loaded():
            print(f"[s_end_v2] Sentinel loaded: {sent_path}")
        else:
            sentinel = None
    if sentinel is None:
        print("[s_end_v2] Sentinel unavailable — sentinel reward = 0")
    ...
    if malom_path and Path(malom_path).exists():
        try:
            ...
            if db.is_available():
                print(f"[s_end_v2] Malom DB loaded (lookahead termination only): {malom_path}")
            else:
                db = None
        except Exception as e:
            print(f"[s_end_v2] Malom DB failed ({e})")
    if db is None:
    ...
    if vn_path and Path(vn_path).exists():
        try:
            ...
            print(f"[s_end_v2] Value net loaded: {vn_path}")
        except Exception as e:
            print(f"[s_end_v2] Value net load failed ({e}) — VN features will be 0")
    else:
        print("[s_end_v2] No value net — VN features will be 0")
    ...
    if gap_path and Path(gap_path).exists():
        try:
            ...
            print(f"[s_end_v2] Gap net loaded: {gap_path}")
        except Exception as e:
            print(f"[s_end_v2] Gap net load failed ({e}) — gap features will be 0.5")
    else:
        print("[s_end_v2] No gap net — gap features will be 0.5")
    ...
    if hdb_path.exists():
        try:
            ...
            print(f"[s_end_v2] HumanDB loaded: {human_db.game_count} games "
                  f"({human_db.entry_count} positions)")
        except Exception as e:
            print(f"[s_end_v2] HumanDB load failed ({e}) — human_freq features will be 0")
    else:
    ...
    print("[s_end_v2] LookaheadAdvisor: 15-ply, 4 signals (h+vn+sent+gap), Malom DB for early-exit")
    ...
    if resume_path is None:
        print("[s_end_v2] No checkpoint found — starting from scratch")
    else:
        print(f"[s_end_v2] Resuming from ({source_tag}): {resume_path}")
    print(f"[s_end_v2] feat_dim={MOVE_FEAT_DIM_WITH_TOPK}, starting game={start_game}, diff={difficulty}")
    ...
    print("[s_end_v2] LookaheadAdvisor: frozen-model driven learner-side (Option C)")
    ...
    if not args.no_position_pool:
        try:
            ...
            print(f"[s_end_v2] Endgame position pool: {len(endgame_pool)} positions")
        except Exception as e:
            print(f"[s_end_v2] Position pool load failed ({e}) — starting from new_game()")

    print(f"[s_end_v2] Starting at game {game_count}, difficulty {difficulty}")
    print(f"[s_end_v2] Self-play ratio {args.self_play_ratio:.0%}, "
    ...
    if not args.no_s1a_warmstart and start_game == 0:
        print(f"[s_end_v2] Running s1a warm-start (pre-RL imitation) from {args.s1a_data}")
    ...
    if args.batch_games > 1:
        ...
        print(f"[s_end_v2] Encoder warmed for {args.batch_games}-game parallel batches")
    ...
    while game_count < args.max_games:
        ...
        if games_since_target_update >= args.update_target_every:
            ...
            games_since_target_update = 0
            print(f"[s_end_v2] Frozen model updated at game {game_count}")
        ...
        for _ in range(max(1, min(args.batch_games, args.max_games - game_count))):
            ...
            _gd = max(DIFF_START, difficulty - 1) if mixed_diff_game else difficulty
            _cfa = not mixed_diff_game
            ...
            if not endgame_pool:
                raise RuntimeError("[s_end_v2] endgame position pool is empty — refusing to train from placement. "
                                   "Remove --no-position-pool or ensure the pool loader finds positions.")
            _sb = endgame_pool[rng.randint(0, len(endgame_pool) - 1)]
            _lc = _sb.turn
            _oc = "B" if _lc == "W" else "W"
            if _gt == "vs_heuristic":
                _tb = _time_budget_for_level(_gd) if args.time_budget <= 0 else args.time_budget
        ...
        for cfg, opponent, result in batch_results:
            ...
            game_difficulty     = cfg.game_difficulty
            counts_for_advance  = cfg.counts_for_advance
            ...
            if result.outcome == WIN_REWARD:
                ...
                if game_count % 10 == 0:
                    print(f"[s_end_v2] {game_count:6d}  r{game_retry_ply:2d} {learner_color} |          | {_coc} ply={confirm_result.ply:3d} | (from ply {game_retry_ply}) {'[learn]' if confirmed else '[skip]'}")
            ...
            if result.outcome != WIN_REWARD and result.retry_board is not None:
                ...
                if game_count % 10 == 0:
                    print(f"[s_end_v2] {game_count:6d} retry {learner_color} |          | {_roc} ply={retry_result.ply:3d} | (from ply {game_retry_ply})")
            ...
            for branch_ply, branch_board, bucket in ordered_candidates:
                ...
                if branch_result.trajectory:
                    ...
                    if game_count % 10 == 0:
                        ...
                        print(f"[s_end_v2] {game_count:6d}  +b  {learner_color} | {bucket:7s} | {_boc} ply={branch_result.ply:3d} | (from ply {branch_ply})")
            ...
            if game_count % args.log_every == 0 and diag_buffer:
                ...
                if last_main:
                    ...
                    print(
                        f"[s_end_v2] game {game_count:6d} | diff {difficulty} | "
                ...
                if win_rate > best_win_rate_at_diff and len(win_history_heuristic) >= 10:
                    ...
                    print(f"[s_end_v2]  → best diff-{difficulty} win rate: {best_win_rate_at_diff:.3f}")
            ...
            if games_at_level >= 20 and games_at_level % 10 == 0:
                ...
                if game_count % 50 == 0:
                    print(f"[s_end_v2] advance-check @ diff {difficulty}: {_adv.reason}")
            if _adv is not None and _adv.should_advance:
                if difficulty >= args.diff_max:
                    print(f"[s_end_v2] *** DONE at diff {difficulty}: {_adv.reason} ***")
                    _advance_done = True
                    break
                else:
                    ...
                    win_history_heuristic.clear()
                    games_at_level = 0
                    print(f"[s_end_v2] *** Advanced to diff {difficulty} (was diff {prev_diff}: "
                          f"score={_adv.score_pct:.3f} P={_adv.p_super:.3f} target={_adv.target:.3f}) ***")
                    win_rate_adv = _adv.score_pct
                    ...
                    if not prev_best.exists():
                        ...
                        print(f"[s_end_v2] Saved best{prev_diff}.pt at advancement (wr={win_rate_adv:.3f})")
                    if prev_best.exists():
                        ...
                        try:
                            ...
                            model.load_state_dict(pol_state, strict=False)
                            print(f"[s_end_v2] Advance-load: value_mlp shape mismatch — policy weights loaded, value head kept")
                        model.to(device)
                        print(f"[s_end_v2] Loaded best{prev_diff}.pt as starting point for diff {difficulty}")
    ...
    torch.save(ckpt, out_dir / "latest.pt")
    print(f"\n[s_end_v2] Done. Games: {game_count}  Best win rate: {best_win_rate:.3f}")
    print(f"[s_end_v2] Checkpoint: {out_dir / 'best.pt'}")


# ── CLI ───────────────────────────────────────────────────────────────────────

def main() -> None:
    p = argparse.ArgumentParser(description="Endgame specialist v2: 15-ply lookahead + gap net")
    p.add_argument("--resume",             default="",   type=str)
    p.add_argument("--auto-resume-best",   action="store_true")
    p.add_argument("--out-dir",  default=str(_ROOT / "learned_ai" / "checkpoints" / "scaffolded" / "s_end_v2"))
