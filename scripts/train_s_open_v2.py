...
def _check_advance(win_history_heuristic: deque, rolling_win: int, difficulty: int) -> bool:
    ...
    Level 4+: threshold ramps 0.51→0.60, draw cap 0.50 (opening draws are
    common due to heuristic playout after placement), min 20 games.
    ...
    n         = len(recent)
    draw_rate = draws / n
    if draw_rate >= (0.55 if early else 0.50):
        return False
...
def _rollout(
    ...
    if specialist_db is not None and learner_boards:
        try:
            ...
            specialist_db.record_game(learner_boards + learner_result_boards, _res, learner_moves_notation, "open", learner_color=learner_color)
        except Exception:
            pass
...
def run(args: argparse.Namespace) -> None:
    ...
    if _imitation_path.exists():
        try:
            ...
            print(f"[s_open_v2] Imitation data loaded: {len(_imitation_data['feat_matrices'])} positions for mixing")
        except Exception as e:
            print(f"[s_open_v2] Imitation data load failed ({e}) — imitation mixing disabled")
        ...
        for cfg, opponent, result in batch_results:
            ...
            if game_count % args.log_every == 0 and diag_buffer:
                ...
                if (len(win_history_heuristic) >= RECOVERY_MIN_GAMES
                    ...
                    if _is_improving:
                        print(f"[s_open_v2] Recovery skipped: AI is improving ({_first_wr:.3f} → {_second_wr:.3f})")
                    else:
